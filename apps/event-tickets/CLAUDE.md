...
| Showcase | `app/app/page.tsx` → `components/EventShowcase.tsx`, `lib/public-events.ts` | The app's home lists public, upcoming events: a swipeable slider of featured posters, then every event as a row. Server-rendered per request (`connection()`); served by the `(visibility, datetime_utc)` index. |
